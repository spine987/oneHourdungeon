...
def get_boxItem():
    global gameMode, tmr
    d  = random.choice([0,0,0,1,1,2,2,3,3,4])
    if d == 0:
...
def main():
    ...
    pygame.init()
    pygame.display.set_caption("優化戰鬥")
    ...
    global changeActCooldown, foodCooldown
    walkCooldown = 0
    disCooldown = 0
    foodCooldown = 0
    changeActCooldown = 0
            ...
            if event.type == KEYDOWN:
                if walkCooldown <= 0:
                    ...
                    def doCooldown(act1, act2):
                        global foodCooldown, changeActCooldown, player
                ...
                for x in range(-13,14):
                    ...
                    try:
                        if dungeon[player.y + y][player.x + x] == 0:
                            ...
                            screen.blit(img_box, [W*(x+5),H*(y+5)])
                        elif dungeon[player.y + y][player.x + x] == 3:
                            screen.blit(img_stair, [W*(x+5),H*(y+5)])
                        elif dungeon[player.y + y][player.x + x] == 4:
                            screen.blit(img_cocoon, [W*(x+5),H*(y+5)])
